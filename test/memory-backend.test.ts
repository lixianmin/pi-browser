...
// Plan 7b T6a 平移：pi FileSystem 适配器（内存后端）单测。
// 契约来源：pi `harness/types.d.ts` 的 FileSystem（17 方法，返回 Result<T, FileError>，不抛）。
// 源：spice `packages/harness/test/session-fs.test.ts`（该文件在 vitest 下经工厂自动落到内存后端，
...
describe('createMemoryFileSystem：路径与读写（Plan 7b T6a）', () => {
...
describe('createMemoryFileSystem：目录、存在性与错误映射（Plan 7b T6a）', () => {
