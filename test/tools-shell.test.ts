// Task 7：Shell 工具契约测试（spec §3.3 表第七行 + §3.2 exec 映射）。
...
describe('Shell tool', () => {
	it('经 ExecutionEnv.exec 跑 busybox，stdout 累积成 content', async () => {
		const out = textOf(await createShellTool({ env: busyboxEnv() }).execute('id', { command: 'echo hello' }));
		expect(out).toBe('hello\n');
	});

	it('退出码非 0 → content 附 exit code，details 透传 exitCode + truncation', async () => {
		const r = await createShellTool({ env: busyboxEnv() }).execute('id', { command: 'exit 3' });
	...
	it('无输出 → (no output)', async () => {
		expect(textOf(await createShellTool({ env: busyboxEnv() }).execute('id', { command: 'true' }))).toBe('(no output)');
	});

	it('命令写文件端到端落进挂载的 fs（inline 路径 run 边界同步）', async () => {
		const fs = createMemoryFileSystem();
		await createShellTool({ env: busyboxEnv(fs) }).execute('id', { command: 'echo written > out.txt' });
	...
	it('timeout 默认 30（秒），可按输入覆盖', async () => {
		...
		const tool = createShellTool({ env: recordingEnv(seen) });
	...
	it('capture 只给 limits，永不设定 spill（spec §6：不支持超限落盘）', async () => {
		const seen: ShellExecOptions[] = [];
		await createShellTool({ env: recordingEnv(seen) }).execute('id', { command: 'echo hi' });
	...
	it('shell:false 的占位 env → shell_unavailable（ExecutionError 原样抛出）', async () => {
		...
		await expect(createShellTool({ env }).execute('id', { command: 'echo hi' }))
			.rejects.toMatchObject({ code: 'shell_unavailable' });
	});

	it('调用前已 abort → aborted（ExecutionError 由 exec 给）', async () => {
		await expect(createShellTool({ env: busyboxEnv() }).execute('id', { command: 'echo never' }, AbortSignal.abort()))
			.rejects.toMatchObject({ code: 'aborted' });
	});

	it('description 如实声明 busybox 语义与不支持项', () => {
		const { description } = createShellTool({ env: busyboxEnv() });
